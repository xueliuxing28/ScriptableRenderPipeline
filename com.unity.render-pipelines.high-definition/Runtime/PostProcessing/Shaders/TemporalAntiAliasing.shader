Shader "Hidden/HDRP/TemporalAntialiasing"
{
    HLSLINCLUDE
        ...
        void FragTAA(Varyings input, out CTYPE outColor : SV_Target0)
        {
            ...
            CTYPE color = FETCH_COLOR(_InputTexture, uv, 0.0, _RTHandleScale.xy);
            CTYPE history = FETCH_COLOR(_InputHistoryTexture, input.texcoord - motionVector, 0.0, _RTHandleScaleHistory.zw);

            CTYPE topLeft = FETCH_COLOR(_InputTexture, uv, -RADIUS, _RTHandleScale.xy);
            CTYPE bottomRight = FETCH_COLOR(_InputTexture, uv, RADIUS, _RTHandleScale.xy);

            CTYPE corners = 4.0 * (topLeft + bottomRight) - 2.0 * color;

            // Sharpen output
    #if SHARPEN
            CTYPE topRight = FETCH_COLOR(_InputTexture, uv, float2(RADIUS, -RADIUS), _RTHandleScale.xy);
            CTYPE bottomLeft = FETCH_COLOR(_InputTexture, uv, float2(-RADIUS, RADIUS), _RTHandleScale.xy);
            ...
            CTYPE unjitteredColor = FETCH_COLOR(_InputTexture, input.texcoord - color.w * jitter, 0.0, _RTHandleScale.xy);
        ...
        void FragExcludedTAA(Varyings input, out CTYPE outColor : SV_Target0)
        {
            ...
            outColor = FETCH_COLOR(_InputTexture, uv, 0.0, _RTHandleScale.xy);
        }
    ENDHLSL
